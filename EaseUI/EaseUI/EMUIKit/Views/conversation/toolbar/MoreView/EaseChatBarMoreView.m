...
#pragma mark - 阅后即焚按钮处理
        _fireButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_fireButton setFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 10 * 2 +CHAT_BUTTON_SIZE + 10, CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE)];
        [_fireButton setTitle:@"阅后即焚" forState:UIControlStateNormal];
        [_fireButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        _fireButton.titleLabel.font = [UIFont systemFontOfSize:9];
        [_fireButton setBackgroundColor:[UIColor lightGrayColor]];
        [_fireButton addTarget:self action:@selector(fireMessageAction:) forControlEvents:UIControlEventTouchUpInside];
        _fireButton.tag = MOREVIEW_BUTTON_TAG + 5;
        _maxIndex = 5;
        [_scrollview addSubview:_fireButton];
    }
